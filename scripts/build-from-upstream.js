...
function parsePlatform(argv) {
  ...
  if (!platform || !["mac-arm64", "mac-x64"].includes(platform)) {
    throw new Error("Usage: build-from-upstream.js --platform <mac-arm64|mac-x64>");
  }
  return platform;
}
...
function main() {
  ...
  return buildMac({ ...BUILD_CONTEXT, platform });
}
